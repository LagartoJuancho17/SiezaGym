...
final class ThemeStore {
    var actual: Theme {
        didSet {
            UserDefaults.standard.set(actual.id, forKey: Self.clave)
            WidgetBridge.actualizarTema(actual.id)
        }
    }
    ...
    static var temaGuardado: String {
        UserDefaults.standard.string(forKey: clave) ?? Theme.porDefecto.id
    }
