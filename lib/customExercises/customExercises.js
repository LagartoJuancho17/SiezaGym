...
export async function listCustomExercises(uid) {
  try {
    ...
    console.warn("[listCustomExercises] Firestore quota/error:", error.message);
    return [];
  }
}
...
export async function getCustomExercise(uid, exerciseId) {
  try {
    ...
    console.warn("[getCustomExercise] Firestore quota/error:", error.message);
    return null;
  }
}
...
export async function createCustomExercise(uid, data) {
  ...
  const weights = data.muscleWeights || {};
  const hasWeights = Object.keys(weights).length > 0;
  if (hasWeights && !isValidMuscleWeights(weights)) {
  ...
  try {
    ...
    const ref = await collection(uid).add({
      ...
      registrationType: data.registrationType || "peso_reps",
      unilateral: !!data.unilateral,
    ...
    console.warn("[createCustomExercise] Firestore error:", error.message);
