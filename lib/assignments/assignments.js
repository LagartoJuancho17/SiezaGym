...
// Firestore devuelve loggedAt como instancia de Timestamp (clase, no objeto
// plano) - Next.js no puede pasar eso de Server a Client Component tal cual.
function serializeExerciseLogs(exerciseLogs) {
  ...
  for (const [index, log] of Object.entries(exerciseLogs)) {
    result[index] = {
      ...log,
      loggedAt: log.loggedAt?.toDate?.().toISOString() || null,
    };
  }
  return result;
}
