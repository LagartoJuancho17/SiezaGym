...
function serializeAssignment(doc) {
  ...
  return {
    ...
    weekNumber: data.weekNumber != null ? Number(data.weekNumber) : null,
    weekLabel: data.weekLabel || (data.weekNumber != null ? `Semana ${data.weekNumber}` : null),
...
export async function assignRoutineToStudent(coachId, studentId, routine, options = {}) {
  if (!coachId || !studentId || !routine) {
    throw new Error("Faltan datos para la asignación.");
  }

  const weekNumber =
    options?.weekNumber != null && options.weekNumber !== ""
      ? Number(options.weekNumber)
      : null;
  const weekLabel =
    options?.weekLabel || (weekNumber != null ? `Semana ${weekNumber}` : null);
  ...
  const alreadyAssignedForWeek = snapshot.docs.some((doc) => {
    const docWeek =
      doc.data().weekNumber != null ? Number(doc.data().weekNumber) : null;
    return docWeek === weekNumber;
  });

  if (alreadyAssignedForWeek) {
    throw new Error(
      weekNumber != null
        ? `Esta rutina ya está asignada a este alumno para la Semana ${weekNumber}.`
        : "Esta rutina ya está asignada a este alumno.",
    );
  }
  ...
  const ref = await getDb().collection(COLLECTION).add({
    ...
    note: options?.note || routine.note || "",
    assignedAt: now,
    lastUsedAt: null,
    exerciseLogs: {},
  });
...
/**
 * Las series que el alumno cargó, con la forma que espera createSession.
 *
 * Los logs vienen indexados por posición del ejercicio en la rutina asignada
 * ("0", "1", ...), así que hay que cruzarlos contra la lista de ejercicios para
 * saber de qué ejercicio es cada uno. Sin React ni Firestore adentro: es la
 * parte de completeAssignmentSession que se puede probar sola.
 */
export function sessionExercisesFromLogs(exercises, exerciseLogs) {
...
export async function completeAssignmentSession(studentId, assignmentId, durationSeconds) {
  ...
  const exercisesForSession = sessionExercisesFromLogs(data.exercises, data.exerciseLogs);

  let summary = { totalSetsCompleted: 0, totalVolumeKg: 0 };
  if (exercisesForSession.length > 0) {
    summary = await createSession(studentId, {
  ...
  await docRef.update({
    ...
    lastUsedAt: FieldValue.serverTimestamp(),
    // Los logs son el borrador de la sesión en curso, no el registro: el
    // registro es el documento de sessions que se acaba de crear. Si no se
    // vacían, el próximo entrenamiento arranca con las series del anterior
    // adentro y las vuelve a contar.
    exerciseLogs: {},
  });
