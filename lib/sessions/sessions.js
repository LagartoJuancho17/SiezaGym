...
// "YYYY-MM-DD" en horario Argentina, sin importar en que zona corre el server.
...
export async function listTrainedDates(userId, { sinceDays = 120 } = {}) {
  try {
    ...
    for (const doc of snapshot.docs) {
      const finishedAt = doc.data().finishedAt?.toDate?.();
      if (!finishedAt) continue;
      dates.add(toLocalDayKey(finishedAt));
    }
    return [...dates];
  } catch (error) {
    console.warn("[listTrainedDates] Firestore quota/error:", error.message);
    return [];
  }
}
...
export async function weeklyVolumeKg(userId) {
  try {
    ...
    const snapshot = await getDb()
      ...
      .where("userId", "==", userId)
      .where("finishedAt", ">=", since)
      // Sin orderBy explicito Firestore ordena ASC por defecto y pide un
      // indice que no esta deployado: la query fallaba y devolvia 0.
      .orderBy("finishedAt", "desc")
      .get();
    ...
    for (const doc of snapshot.docs) {
      const data = doc.data();
      if (typeof data.totalVolumeKg === "number") {
        total += data.totalVolumeKg;
      }
    }
    return Math.round(total);
  } catch (error) {
    console.warn("[weeklyVolumeKg] Error:", error.message);
    return 0;
  }
}
