...
function sanitizeExercises(exercises) {
  ...
  return exercises.map((exercise, index) => ({
    ...
    group: exercise.group ? String(exercise.group).trim() : "",
    groupColor: exercise.groupColor ? String(exercise.groupColor).trim() : "",
...
/**
 * Marca la rutina como usada recién ahora.
 *
 * /rutinas y la portada ordenan por lastUsedAt, así que sin esto la rutina que
 * entrenaste hoy sigue apareciendo donde estaba: el campo existía desde el
 * principio y nunca se escribía.
 */
export async function markRoutineUsed(userId, routineId) {
  const docRef = getDb().collection(COLLECTION).doc(routineId);
  const doc = await docRef.get();
  if (!doc.exists || doc.data().ownerId !== userId) {
    throw new Error("Rutina no encontrada.");
