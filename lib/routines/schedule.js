...
export function groupByMonthAndWeek(items, toParts) {
  ...
  for (const item of items || []) {
    ...
    const week = item.weekNumber != null ? Number(item.weekNumber) : weekOfMonth(parts.day);
    const weekLabelText = item.weekLabel || `Semana ${week}`;
    if (!monthEntry.weeks.has(week)) {
      monthEntry.weeks.set(week, { week, label: weekLabelText, items: [] });
    }
