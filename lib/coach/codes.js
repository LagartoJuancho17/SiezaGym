...
export async function redeemCode(code, studentId) {
  ...
  const normalized = clean.length === 6 ? `${clean.slice(0, 3)}-${clean.slice(3)}` : raw;

  const db = getDb();
  const primaryRef = db.collection(COLLECTION).doc(normalized);
  const fallbackRef = raw !== normalized ? db.collection(COLLECTION).doc(raw) : null;

  try {
    const result = await db.runTransaction(async (tx) => {
      let doc = await tx.get(primaryRef);
      let targetRef = primaryRef;

      if (!doc.exists && fallbackRef) {
        const altDoc = await tx.get(fallbackRef);
        if (altDoc.exists) {
          doc = altDoc;
      ...
      if (!expiresAt || expiresAt.getTime() < Date.now()) {
        tx.update(targetRef, { status: "expired" });
        return {
          success: false,
